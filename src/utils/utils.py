...
def random_crop(image, mat_label, size):
    _, H, W = image.shape
    th, tw = size
    # ensure valid range; if equal, choose 0; if larger image, sample within [0, H-th]
    start_h = 0 if H <= th else int(torch.randint(H - th + 1, (1,))[0])
    start_w = 0 if W <= tw else int(torch.randint(W - tw + 1, (1,))[0])
    image = image[..., start_h:start_h + th, start_w:start_w + tw]
    mat_label = mat_label[..., start_h:start_h + th, start_w:start_w + tw]
...
def _ensure_min_size(image, mat_label, size):
    """
    If input smaller than desired size, resize image (bilinear) and label (nearest)
    to the desired size to avoid invalid crop ranges.
    """
    _, H, W = image.shape
    th, tw = size
    if H < th or W < tw:
        image = F.interpolate(image.unsqueeze(0), size=size, mode='bilinear', align_corners=True)[0]
        mat_label = F.interpolate(mat_label.unsqueeze(0), size=size, mode='nearest')[0]
    return image, mat_label
...
def augment_data(image, mat_label, size, flip=True, test=False):
    ...
    image, mat_label = _ensure_min_size(image, mat_label, size)
    if test:
